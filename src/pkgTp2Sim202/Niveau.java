...
public class Niveau {
    ...
    protected Tuile[][] lire(int niveau) {
        ...
        try {
            ...
            while (ligne != null) {
                if (ligneLu == 0) {
                    ...
                    positionAutres=LigneEnArray(0,tableauTemporaire,positionAutres,tuiles);//voir la méthode
                    // en question
                    if (positionAutres.contains('#')) {//compter la grandeur de la carte (à partir de #)
                        ...
                        while (ligne != null) {//lire la ligne, ajouter 1 à largeur puis répéter jusqu'à ce qu'il
                            // n'y a plus rien
                            largeur++;
                            ligne = fichier.readLine();
                        }
            ...
            carte = new Tuile[largeur][longueur];//créer un tableau 2d avec la grandeur trouvé (seulement les murs
            // et planchers)
            ...
            while (ligne != null) {//remplir le tableau 2d ici
                ...
                positionAutres=LigneEnArray(0,tableauTemporaire,positionAutres,tuiles);//si on est rendu
                // à la ligne de la carte, alors il ne devrait pas avoir de virgule
                    ...
                    if (positionAutres.get(i).equals('#')) {
                        while (ligne != null) {
                            for (int t = 0; t < largeur; t++) {
                                carte=lireNiv(t,positionAutres,longueur,carte);//pour chaque ligne à partir de
                                // celui ou il y a #, les données sont en paramètres
                                ligne = fich.readLine();//ligne suivante
                                if (ligne == null) {//s'il n'y a plus rien
                                    break;
                                }
                                tableauTemporaire = ligne.split(",");//refait la même chose, avec la deuxième
                                // ligne
            ...
            while (ligne != null) {
                if (ligneLu != 0) {//ce n'est 100% pas la première ligne qu'il y a les données de monstres ou de
                    // trésors
                    String[] tableauTemporaire = ligne.split(",");
                    positionAutres=LigneEnArray(0,tableauTemporaire,positionAutres,tuiles);
                    //la même formule: passe les premiers charactères avant la virgule en Arraylist
                    for (int i = 0; i < positionAutres.size(); i++) {
                        if (positionAutres.get(i).equals(':')) {//si il y a ':', alors c'est sûrement quelque chose à
                            // placer
                            if (positionAutres.get(0).equals('m')) {//si le premier charactère du liste est m, alors
                                // c'est monstre
                                donnesMonstres=CoordonnesPlusieusDonnees(convertisseur,positionAutres,i,
                                        tableauTemporaire,tuiles,donnesMonstres);
                                ...
                                }//à peu près la même chose que monstre: vérifier s'il est un chiffre ou un nombre,
                                // puis le range dans une liste
                                ...
                                /*créer un nouveau tableau avec une taille de tableau temporaire moins 2 (moins
                                l'espace pour les coordonnées x et y)
                                donc si c'est un message avec virgule, la taille est de 2. Sinon, sa taille est de 1.*/
                                for (int r = tableauTemporaire.length - 1; r > 1; r--) {//par exemple, le
                                    /* tableauTemporaire est de taille 4: {"5","8","Allo","ça va?"}
                                    r= 3, temporaire à 0= ça va (tableauTemporaire à la position 3), q+1, r=2,
                                    temp. à 1= Allo (tabTem^. à la position 2), q+1, r=1, sort du for*/
                                ...
                                for (int r = 0; r < temporaire.length; r++) {
                                    /*r=0, messagePancarte.add (temporaire position (temporaire.lengh-a, donc 2-1=1)),
                                     messagePancarte.get(0)=Allo, a+1=2
                                    r=1, messageP.add (temp. position (2-2))m messageP.get(1)=ça va, a+1=3
                                    r=2: sort du for (temporaire.lengh=2)
                                    Bien sûr si c'est sans virgule, le length de temporaire =1 donc il sort après
                                     une fois seulement
                                    */
                                ...
                                //trésor
                                donnesTresor=CoordonnesTresor(convertisseur,positionAutres,i,tableauTemporaire,
                                        tuiles,donnesTresor);
                            } else if (positionAutres.get(0).equals('t') && positionAutres.get(1).equals('e')) {
                                //téléporteur
                                donnesTp=CoordonnesPlusieusDonnees(convertisseur,positionAutres,i,tableauTemporaire,
                                        tuiles,donnesTp);
                                //la même méthode que monstre mais au lieu de nbdeVie et attaque, c'est x et y
        ...
        if (donnesTp.size()!=0) {//car il est possible qu'il n'a aucune teleporteur
            for (int i=0; i<donnesTp.size(); i+=4){//bond de 4 vu qu'il y a 4 infos par téléporteur
                ...
                /*carte à la position y x (dans la liste c'est x y donc c'est inversé)=téléporteur (x et y de
                 desitination (situé après x et y position))
                exemple: dans une lite 4,5,10,14, c'est carte [5][4]=new teleporteur (10,14) 4 et 5 sont la
                 position du * et 15 et 14 sont les destinations
                */
            }
        }
     ...
     * @param tableauTemporaire le tableau à transformer
     * @param positionAutres un Arraylist temporaire
    ...
    private ArrayList<Integer> CoordonneesAdlez (String ligne, ArrayList <Integer> positionJoueur){
        for (int i = 0; i < 2; i++) {//méthode qui add les coordonnées de Adlez
            ...
            // x et y de Adlez
            positionJoueur.add(i, Integer.parseInt(positionString[i]));//le transforme en int en le met dans un
            // Arraylist
        }return positionJoueur;
    }
     ...
     * @param t le int dans la boucle for qui n'est pas dans cette méthode
     * @param positionAutres liste qui a les charactères du fichier
    ...
    private Tuile[][] lireNiv(int t,ArrayList <Character> positionAutres,int longueur,Tuile [][]carte){
                            for (int a = 0; a < longueur; a++) {//avance de gauche à droite
                                if (positionAutres.get(a).equals('#')) {
                                    ...
                                    // sont des classes qui sont mis dedans
     ...
     * @param convertisseur un tableau char temporaire
     * @param positionAutres liste de char qui contient l'information à analyser
     * @param i int qui décide quoi prendre dans la liste
     * @param tableauTemporaire tableay de string qui stocke les données du fichier temporairement
     * @param tuiles tableau char utilisé dans LigneEnArray
     ...
     * @param convertisseur tableau char temporaire
     * @param positionAutres liste char avec l'information à analyser
     * @param i int qui situe la position de la liste
     * @param tableauTemporaire tableau String temporaire
     * @param tuiles tableau char pour ligneEnArray
    ...
    CoordonnesPlusieusDonnees(char []convertisseur,ArrayList<Character>positionAutres,
        ...
        convertisseur[0] = positionAutres.get(positionAutres.size() - 2);
        //dans un tableau, ranger l'avant-dernier char de la liste
        if (convertisseur[0] >= 48 && convertisseur[0] <= 57) {
            //il devrait être ':' s'il est une coordonnée à un chiffre. Sinon,
            convertisseur[0] = positionAutres.get(i + 1);
            /* le i dépend de la situation. Dans lire() le i est la position
            ou il y a ':' (voir environ ligne 93). Donc le char. après : est sûrement un chiffre
            */
            String test = Character.toString(convertisseur[0]);
            //le premier chiffre est passé en String pour mieux le coller après
            convertisseur[0] = positionAutres.get(i + 2);
            //si c'est un nombre à deux positions alors il devrait avoir 2 char. après : au lieu d'un
            test += Character.toString(convertisseur[0]);
            //ajoute à ce String test le deuxième chiffre
            donnes.add(Integer.parseInt(test));
            //donnees est une liste int. Donc ajouter des nombres n'a pas de problème.
        } else {
            convertisseur[0] = positionAutres.get(i + 1);
            //sinon simplement mettre le chiffre dans un tableau et le convertir en int
            donnes.add(Character.getNumericValue(convertisseur[0]));
        }
        positionAutres=LigneEnArray(1,tableauTemporaire,positionAutres,tuiles);
        //pour les coordonnées en y, il sont après une virgule donc à la deuxième place du tableau
        // (chiffre=1 au lieu de 0)
        if (positionAutres.size() >= 2) {
            //positionAutres à la deuxième position est quelque chose du genre de {'1','5'} (y=15)
            convertisseur[0] = positionAutres.get(0);
            String test = Character.toString(convertisseur[0]);
            convertisseur[0] = positionAutres.get(1);
            //j'utilise 1 et 0 au lieu de i car il ne peut avoir qu'une ou deux places.
            test += Character.toString(convertisseur[0]);
            donnes.add(Integer.parseInt(test));
        } else {
            convertisseur[0] = positionAutres.get(0);
        ...
        //position 3: pour monstre c'est son nombre de vie
        if (positionAutres.size() >= 2) {
            convertisseur[0] = positionAutres.get(0);
            String test = Character.toString(convertisseur[0]);
            convertisseur[0] = positionAutres.get(1);
            ...
            convertisseur[0] = positionAutres.get(0);
            donnes.add(Character.getNumericValue(convertisseur[0]));
        }//passé dans une liste donneesMonstre pour faciliter la suite (créer une instance monstre par
        // exemple, je peux chercher ses donnes directement avec cette liste)
        ...
        return donnes;//retourne quelque chose du genre {12, 6, 2, 4, 15, 5, 1, 4} 12=x 6=y 2=nb de vie et
        // 4=attaque. 15=x 5=y 1=nb de vie et 4=attaque.
